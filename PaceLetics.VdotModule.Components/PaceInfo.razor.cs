...
Sie liegt deutlich unter der Schwelle und wird ueberwiegend ueber den Fettstoffwechsel getragen. 
Ein grosser Teil deiner Wochenkilometer sollte in diesem Tempo stattfinden. 
...
Sie ist ueberwiegend aerob, aber kohlenhydratintensiver als E-Pace. 
Viele empfinden hier einen stabilen Flow. 
Fuer reines Grundlagentraining ist sie zu schnell.";

        private string TText = @"Die T-Pace liegt nahe deinem 10-km-Tempo und verbindet hohe aerobe und anaerobe Anteile. 
Dieses Tempo kannst du ungefaehr eine Stunde halten. 
Fortgeschrittene nutzen es fuer laengere Intervalle, Einsteigende fuer kuerzere Abschnitte.";

        private string IText = @"Die I-Pace liegt deutlich oberhalb der Schwelle und setzt starke Reize auf deine VO2max. 
Intervalle sind kurz, die Pausen zu kurz fuer eine komplette Erholung.";

        private string RText = @"Die R-Pace ist sehr schnell und technisch anspruchsvoll. 
Sie verbessert Laufoekonomie, Kraft und Koordination. 
Intervalle sind kurz, Pausen lang. 
Vor allem fuer erfahrene Laeufer geeignet.";
    }
}
